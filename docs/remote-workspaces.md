...
Delivering the prompt requires waiting for claude to become ready in the pane first. That wait defaults to 15s for `--on` dispatches (vs. 5s for local workspaces) since claude reliably takes longer to render on a fresh remote host. `CANOPY_PROMPT_PHASE_BUDGET=30s` overrides either default, but only when set in the process that actually runs the wait: for local workspaces that's your own shell, but `--on` dispatch runs the wait inside the remote canopy process over SSH, which doesn't currently inherit env vars set on the invoking laptop — setting `CANOPY_PROMPT_PHASE_BUDGET` before `canopy new --on ...` on your laptop has no effect. Forwarding it to remote dispatches is tracked in TODOS.md.
