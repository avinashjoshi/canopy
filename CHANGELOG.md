...
- **Attaching to a remote workspace could fail with `mosh-server: execvp: --: No such file or directory`.** The security fix above added a `--` before the mosh target, but a second, pre-existing `--` between the target and the remote command was still there too — mosh has no such separator in its own syntax, so that leftover token became the literal first argument of the command mosh-server tried to run. Fixed by removing the redundant separator; exactly one `--` (protecting the target) belongs in the invocation.
- **Creating a workspace from a `--remote`-pinned or unregistered-host row failed with "this directory isn't a canopy project yet."** The remote dispatch had no cwd to `cd` into: the laptop's host registry has no `Projects` map for a raw SSH target (it was never `canopy host add`-ed), so the existing cwd lookup always came up empty. Remote rows now also carry their project's absolute path straight from that host's own `canopy ls --json` response, and the dispatcher falls back to it when the registry doesn't have an answer.
- **`canopy host add --interactive`'s `ssh-copy-id` offer wasn't covered by the `ssh-copy-id` validation fix above.** The Hosts tab's own ssh-copy-id flow was fixed, but the interactive wizard's form only checked its SSH-target field for non-empty — an option-shaped target typed there reached `ssh-copy-id` unguarded. It now validates at the same point the Hosts tab does, before invoking `ssh-copy-id`.
...
The first: pressing `e` to expand the "+ N idle projects" roll-up only ever expanded the local host. v0.21.11.0 (the fix that made the v0.21.7.0 feature fire) introduced per-host independent expansion — pressing `e` toggled the idle group on the host the cursor was in. That sounded clean until you tried it against a remote host whose rows were ALL idle (every project's `(main)` is the only row, and the host hasn't seen recent activity). Those rows are hidden by default; the cursor can never land on them; pressing `e` from a local row only ever flipped local. The user-visible symptom: "idle expand works for laptop, never for tower." Fix: `e` now toggles every host with idle rows in lockstep. If any host with idle is currently expanded, the press collapses all; otherwise it expands all. The per-host map structure stays — the keymap just writes every key in unison — so the renderer's per-host roll-up still draws independently. Test pinning that pressing `e` while the cursor is on a local row also expands a tower host whose only rows are idle.

The second: `canopy ls --json` over SSH was timing out on tower (Avi's home server, WAN/Tailscale) and that timeout was painting tower as a broken canopy install (`✗` red, "broken") instead of a slow-but-healthy remote (`○` gray, "unreachable"). Two pieces broke together:

- The per-host refresh deadline was 3s. SSHCmdBatch sets `ConnectTimeout=5` on its own. The very first refresh after the laptop wakes has to do a cold SSH handshake (no ControlMaster socket yet), and on a slow link 3s reliably killed the SSH child BEFORE the ControlMaster socket could land — so the next refresh ALSO had to do a cold handshake, ALSO timed out, forever. Pi worked because its handshake stayed under 3s on local LAN. Tower didn't.
- When the ctx deadline killed the SSH child, `cmd.Run()` returned the OS-level error string `signal: killed`. The Hosts-tab classifier in `BuildRows` does substring matching: "timeout", "Connection refused", "no route to host" → StatusOffline. None of those matched. The error fell through to the catchall → StatusBroken (red ✗ "broken"). A healthy remote with a slow first handshake looked like a crashing canopy.

Both fixes landed together: bump the default per-host deadline from 3s to 8s so the cold SSH handshake has room to land its ControlMaster socket (subsequent refreshes are sub-second on the warm path), and detect `errors.Is(ctx.Err(), context.DeadlineExceeded)` explicitly in `refreshOneHost` to emit `ssh <host> canopy ls --json: timeout after <duration>` instead of leaking `signal: killed`. The classifier's "timeout" substring catches that, the host shows offline (the right state for "we'll try again next tick"), and cached rows from `remotes-cache.json` keep rendering underneath while we retry.

### Fixed

- **Idle roll-up expands remote hosts too.** Pressing `e` in the workspaces tab now flips every host with idle projects in lockstep. The per-host expansion map is still the underlying structure (the renderer iterates it per host to draw "+ N idle projects · e expand/collapse"), but the keymap writes every key together so a host whose only rows are idle isn't structurally unreachable. Removes the regression introduced in v0.21.11.0 where per-host toggling looked clean in tests but left users unable to expand a fully-idle remote.

- **Slow `canopy ls --json` refresh no longer paints the connection as broken.** Two changes in `internal/host/refresh.go`: per-host deadline bumped from 3s to 8s so the cold SSH handshake (where ConnectTimeout=5 alone eats most of a 3s budget) has room to land its ControlMaster socket; and a new `errors.Is(ctx.Err(), context.DeadlineExceeded)` branch in `refreshOneHost` emits `timeout after <duration>` instead of letting the OS-level `signal: killed` leak into the error. That substring is what `hosts.BuildRows` matches to assign StatusOffline (the right state — reachable in principle, just slow this tick; cached rows keep rendering). Without it, slow remotes fell through to the StatusBroken catchall and rendered "✗ broken" even though SSH and canopy worked fine on the host. Regression test covers every branch of the classifier (timeout, connection refused, permission denied, canopy-not-found, catchall) and a unit test on `refreshOneHost` pins the contract that ctx-deadline-exceeded produces a "timeout" error string.
...
**Remote refresh stayed silent on the Workspaces tab.** The Hosts tab has shown a per-host spinner during fan-out since v0.21.1, but the Workspaces tab only added the cyan `loading…` placeholder row for hosts with zero rows yet — once a host returned even a synthetic `(main)` row, subsequent refreshes were invisible. On flaky links or first-launch fan-outs that take 2–3s per host, registered hosts looked "gone" until they suddenly weren't. The Workspaces tab now decorates each remote host section header with the same braille spinner glyph during active refresh, animating in lockstep with the Hosts tab via the shared `hostsSpinnerFrame`. Spinners clear automatically when `remoteRowsLoadedMsg` lands.

**Workspaces stayed on namegen names forever after the first launch.** The `rename_suggested` hint only fired when a workspace had at least one commit past `origin/main`, which created a dead spot: the agent gathers intent on its first turn, starts editing files, but doesn't commit yet → no hint → the resume-launch *delta* briefing only re-emits hints that fire, so the rename directive that the fresh-launch briefing nudged once never gets re-nudged. The workspace ends up running for hours with a meaningless name like `fair-comet` even though `*7` files have been modified. The detector now also fires when tracked files have unstaged or staged modifications. Untracked files (build artifacts, `scripts.setup` byproducts, IDE caches) are explicitly excluded — counting them would spam the hint forever in workspaces with chatty tooling.

### Added

- **Per-host spinner glyph on the Workspaces tab section header.** `internal/ui/projectlist/projectlist.go` adds a `loadingHosts map[string]bool` on the projectlist Model plus `SetLoadingHosts` / `LoadingHosts` methods. `renderTable` appends a cyan braille frame next to the host name when the host is in the loading set. Distinct from the v0.22 `Loading=true` placeholder row mechanism (which only fires for hosts with zero rows) — the header spinner also fires for hosts with cached rows that are mid-refresh, so the "we're checking" signal is visible alongside stale data. Wired through `pushLoadingHosts()` in `internal/ui/model.go:refresh()` on dispatch and cleared on `remoteRowsLoadedMsg` in `internal/ui/update.go`. 4 projectlist tests + 5 model tests cover header rendering, frame animation, local-section exemption, refresh start/end transitions, and the empty-hostList no-op.

### Fixed

- **`rename_suggested` hint now fires on tracked uncommitted changes, not just commits past main.** `internal/lifecycle/rename.go:detectRenameSuggested` checks both `gitCommitsPastDefault` and a new `gitTrackedDirtyCount` helper (in `internal/lifecycle/git_stats.go`) that runs `git status --porcelain` minus `?? ` lines so untracked noise doesn't qualify as "intent gathered." Message phrasing reflects which kind of progress was found ("N commit(s) past main" vs "N uncommitted file(s)"). Three new lifecycle tests pin the tracked-dirty-fires-hint, untracked-only-stays-silent, and truly-clean-returns-nil branches.
- **`R` (retry) on a synthetic Loading placeholder no longer dispatches `canopy retry --on <host> "" --force` against the remote.** Caught by adversarial review: the v0.22 work added `|| row.Loading` guards to `enter`/`o`/`d`/`K` action handlers but missed `actionRetry`, which would have fired a malformed remote retry verb with `force=true` because the placeholder's empty `Name` doesn't equal `StatusBroken`. Added the guard in `internal/ui/update_retry.go:21` plus a regression test in `internal/ui/model_test.go`.
- **`i` (inspect) on a synthetic Loading placeholder no longer surfaces the misleading "remote inspect isn't supported yet — run `canopy inspect ` ..." error with an empty workspace name.** Mirrors the retry guard above; fix in `internal/ui/drawer.go:42` plus a regression test in `internal/ui/drawer_test.go`.
