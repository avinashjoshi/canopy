...
type Workspace struct {
	...
	// NameAutoGenerated is true when the workspace name came from
	// namegen (the user passed "" to canopy new and got a random
	// adjective-noun back). Drives the briefing's rename directive:
	// auto-named workspaces want the "rename to reflect intent ASAP"
	// nudge; user-chosen names (and PR/issue/branch source variants
	// where the branch name is a contract) shouldn't get pressured
	// to rename.
	NameAutoGenerated bool `json:"name_auto_generated,omitempty"`
